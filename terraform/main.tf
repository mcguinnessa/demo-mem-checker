...
resource "aws_ecs_task_definition" "memchecker_task" {
  family                   = "memchecker-task" # Naming our first task
  container_definitions    = <<DEFINITION
  [
    {
      "name": "mem-checker-task",
      "image": "mcguinnessa/demo-mem-checker@${var.docker_sha}",
      "essential": true,
      "memory": 512,
      "cpu": 256,
      "environment": [
...
resource "aws_ecs_service" "memchecker_service" {
  name            = "memchecker-service"                             # Naming our first service
#  cluster         = "${aws_ecs_cluster.mongo_wrapper_cluster.id}"             # Referencing our created Cluster
  ...
  task_definition = "${aws_ecs_task_definition.memchecker_task.arn}" # Referencing the task our service will spin up
  launch_type     = "FARGATE"
...
#  depends_on = [aws_lb_listener.listener]
  ...
  network_configuration {
    ...
    security_groups  = ["${aws_security_group.memchecker_service_security_group.id}"] # Setting the security group
  }
}

resource "aws_security_group" "memchecker_service_security_group" {
  ingress {
    from_port = 0
    to_port   = 0
    protocol  = "-1"
